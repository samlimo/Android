...
public class HistoricalListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		builder.setAdapter(adapter, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				/*
				 * if (which == 2) { googleMap(mPosition); }
				 */
				if (which == 3) {
	...
	private void setListData() {
		...
		for (int i = 0; i < placesList.size(); i++) {
			Historical historical = placesList.get(i);
	...
	public void editData(Integer ePosition) {
	  ...
	  public void googleMap(Integer ePosition) { Intent mEditIntent = new
	  Intent(getApplicationContext(), com.ftfl.historicalplace.map.ShowMapActivity.class); Long mId =
	  Long.parseLong(mIdList.get(ePosition)); mEditIntent.putExtra("mId",
	    mId.toString()); startActivity(mEditIntent); }
